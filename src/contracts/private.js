...
const contractSeed = async () => {
  ...
  let contract = new ethers.Contract(privateAddress, privateABI, signer);
  return contract;
};
...
export const checkBalance = async (owner) => {
  ...
  const amount = balance[0]
    .toString()
    .split(",")
    .map((ele) => parseFloat(ethers.utils.formatUnits(ele.toString(), 18)));
...
export const checkAmount = async (owner) => {
  ...
  const amount = balance[0]
    .toString()
    .split(",")
    .map((ele) => parseInt(ethers.utils.formatUnits(ele.toString(), 18)));
  return amount.reduce((a, b) => a + b);
